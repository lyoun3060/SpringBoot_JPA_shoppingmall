...
public class CartService {
    ...
    public Long addCart(CartItemDto cartItemDto, String email){
        ...
        //현제 상품이 장바구니에 이미 들어가 있는지 조회
        CartItem savedCartItem = cartItemRepository.findByCartIdAndItemId(cart.getId(), item.getId());

        //장바구니에 이미 있던 상품일 경우 기존 수량에 현재 장바구니에 담을 수량만큼을 더해줌
        if(savedCartItem != null){
            savedCartItem.addCount(cartItemDto.getCount());
            return savedCartItem.getId();
        } else {
            //장바구니에 엔티티, 상품엔티티, 장바구니에 담을 수량을 이용하여 CartItem엔티티를 생성
    ...
    public List<CartDetailDto> getCartList(String email){
        ...
        //장바구니에 상품을 담지 않은 경우, 장바구니 엔티티가 없으므로 빈리스트를 반환
        if(cart == null){
    ...
    public boolean validateCartItem(Long cartItemId, String email){
        ...
        //현재로그인한 회원과 장바구니 상품을 저장한 회원이 다른경우 false, 그 반대면 true
    ...
    public Long orderCartItem(List<CartOrderDto> cartOrderDtoList, String email){
        ...
        //장바구니 페이지에서 전달받은 데이터를 이용하여 주문 로직으로 전달할 orderDto객체를 만듬
